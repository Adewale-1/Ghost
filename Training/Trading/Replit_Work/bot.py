...
def calculate_quantity(symbol, percentage):
    ...
    pair_info = next(
        (item for item in exchange_info["symbols"] if item["symbol"] == symbol), None
    )
    ...
    if pair_info:
        ...
        quantity_precision = (
            len(str(lot_size_filter["stepSize"]).rstrip("0").split(".")[1])
            if "." in str(lot_size_filter["stepSize"])
            else 0
        )
        ...
        price_precision = (
            len(str(price_filter["tickSize"]).rstrip("0").split(".")[1])
            if "." in str(price_filter["tickSize"])
            else 0
        )
...
def place_order(symbol, side, leverage, percentage):
    ...
    try:
        ...
        print(f"Error placing {side} order: {e.message}")
        return None
...
def close_order(symbol, side):
    ...
    try:
        ...
        print(
            f"Processing Client Order ID: {order['clientOrderId']} at opened position: {order['side']}"
        )
        if order["status"] == client.ORDER_STATUS_FILLED:
            ...
            print(f"Closed {side} order.")

    except BinanceAPIException as e:
        # send_telegram_message(f"Error closing {side} order: {e.message}")
        print(f"Error closing {side} order: {e.message}")
...
#             short_ema = talib.EMA(np.array(close_prices), timeperiod=short_ema_period)[
#                 -1
#             ]
...
#                     print(
#                         f"Placed BUY order at {latest_price}. Qty: {percentage}, Short EMA: {short_ema}, Long EMA: {long_ema}"
#                     )
...
#                     print(
#                         f"Placed SELL order at {latest_price}. Qty: {percentage}, Short EMA: {short_ema}, Long EMA: {long_ema}"
#                     )
...
#             print(
#                 f"Balance: ${balance} Earning: ${earning} Running: ${latest_price}. Qty: {percentage}, Short EMA: ${short_ema}, Long EMA: ${long_ema}"
#             )
...
#             print("Timeout error when communicating with Binance's API. Retrying...")
#             continue
...
if __name__ == "__main__":
    ...
    print(
        f"Starting bot for {symbol} with Short EMA: {short_ema_period}, Long EMA: {long_ema_period}, Interval: {interval}, Leverage: {leverage}, Percentage: {percentage}"
    )
