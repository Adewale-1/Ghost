...
def check_hs_pattern(
    extrema_indices: List[int], data: np.array, i: int, early_find: bool = False
) -> HSPattern:
...
def get_pattern_return(
    data: np.array, pat: HSPattern, log_prices: bool = True
) -> float:
...
def plot_hs(candle_data: pd.DataFrame, pat: HSPattern, pad: int = 2):
    ...
    mpf.plot(
        data,
        alines=dict(
            alines=[l0, l1, l2, l3, l4, l5, neck],
            colors=["w", "w", "w", "w", "w", "w", "r"],
        ),
        type="candle",
        style="charles",
        ax=ax,
...
if __name__ == "__main__":
    ...
    # data = data[data > 0]  # Keep only positive values
    # data = np.log(data)
